...
class TestRTKPlugin:
    ...
    def test_rtk_rewrite_handles_timeout(self):
        ...
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="rtk", timeout=2)):
            result = _rewrite_terminal_command(
                tool_name="terminal",
                args={"command": "git status"},
            )
    ...
    def test_rtk_rewrite_env_disable(self, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_RTK_DISABLE", "1")
        result = _rewrite_terminal_command(
            tool_name="terminal",
            args={"command": "git status"},
        )
...
class TestHandleFunctionCallArgMutation:
    ...
    def test_terminal_command_rewritten_by_plugin(self):
        ...
        with (
            ...
            patch("hermes_cli.plugins.get_pre_tool_call_block_message",
                  return_value=(None, [{"action": "rewrite", "args": {"command": "rtk git status"}}])),
        ):
    ...
    def test_block_takes_precedence_over_rewrite(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}') as mock_dispatch,
            patch("hermes_cli.plugins.get_pre_tool_call_block_message",
                  return_value=("forbidden", [{"action": "block", "message": "forbidden"}])),
        ):
            result = handle_function_call(
                "terminal", {"command": "rm -rf /"},
                task_id="t1",
